...
def raw_hybrid_solver(reuse_yices_solver):
    claripy.backends.yices.reuse_yices_solver = reuse_yices_solver
    ...
    old_solve_count = claripy.backends.yices.solve_count
    assert sorted(s.eval(x, 20)) == [0, 2, 4, 6, 8, 10]
    assert claripy.backends.yices.solve_count == old_solve_count
...
def raw_replacement_solver(reuse_yices_solver):
    claripy.backends.yices.reuse_yices_solver = reuse_yices_solver
...
def raw_solver(solver_type, reuse_yices_solver):
    claripy.backends.yices.reuse_yices_solver = reuse_yices_solver
    ...
    if isinstance(s, claripy.frontend.mixin.ModelCacheMixin):
        count = claripy.backends.yices.solve_count

        s = solver_type()
        x = claripy.BVS("x", 32)
        s.add(x == 10)
        assert s.satisfiable()
        assert claripy.backends.yices.solve_count == count
...
def raw_solver_branching(solver_type, reuse_yices_solver):
    claripy.backends.yices.reuse_yices_solver = reuse_yices_solver
...
def raw_combine(solver_type, reuse_yices_solver):
    claripy.backends.yices.reuse_yices_solver = reuse_yices_solver
...
def raw_composite_solver(reuse_yices_solver):
    claripy.backends.yices.reuse_yices_solver = reuse_yices_solver
    ...
    if isinstance(s._template_frontend, claripy.frontend.mixin.ModelCacheMixin):
        ...
        count = claripy.backends.yices.solve_count
        assert s.satisfiable()
        assert claripy.backends.yices.solve_count == count + 3
        assert list(s.eval(x + y, 1)) == [3]
        assert claripy.backends.yices.solve_count == count + 3


def raw_minmax(reuse_yices_solver):
    claripy.backends.yices.reuse_yices_solver = reuse_yices_solver
...
def raw_composite_discrepancy(reuse_yices_solver):
    claripy.backends.yices.reuse_yices_solver = reuse_yices_solver
...
def raw_ancestor_merge(solver, reuse_yices_solver):
    claripy.backends.yices.reuse_yices_solver = reuse_yices_solver
...
def raw_unsat_core(solver, reuse_yices_solver):
    claripy.backends.yices.reuse_yices_solver = reuse_yices_solver
